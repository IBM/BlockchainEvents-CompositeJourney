...
function closeBidding(closeBidding) {
    ...
    return getParticipantRegistry('org.acme.product.auction.Seller')
           ...
           .then(function(){
               // Generate event
               var event = getFactory().newEvent('org.acme.product.auction', 'TradeEvent');
               // Set properties
...
function publishListing(listing) {
    ...
    return getAssetRegistry('org.acme.product.auction.ProductListing')
            .then(function (registry) {
                ...
                productListing = factory.newResource('org.acme.product.auction', 'ProductListing',Math.random().toString(36).substring(3));
            ...
            .then(function(){
                // Generate event
                var event = factory.newEvent('org.acme.product.auction', 'TradeEvent');
                // Set properties
...
function addProduct(newproduct) {
    var product = getFactory().newResource('org.acme.product.auction', 'Product', Math.random().toString(36).substring(3));
